...
router.post("/:id/email", auth, async (req, res) => {
  try {
    ...
    let cleanHtml = guide.generatedHtml;
    cleanHtml = cleanHtml.replace(/<style[\s\S]*?<\/style>/gi, "");
    cleanHtml = cleanHtml.replace(/text-shadow\s*:\s*[^;"']+;?/gi, "");
    cleanHtml = cleanHtml.replace(/\bcolor\s*:\s*[^;"']+[;]?/gi, "");
    cleanHtml = cleanHtml.replace(/background(?:-color)?\s*:\s*[^;"']+[;]?/gi, "");
    cleanHtml = cleanHtml.replace(/filter\s*:\s*[^;"']+;?/gi, "");
    cleanHtml = cleanHtml.replace(/-webkit-text-stroke\s*:\s*[^;"']+;?/gi, "");
    
    // Wrap in email-friendly HTML with inline styles for maximum compatibility
    const emailHtml = `
<!DOCTYPE html>
<html>
<head>
...
<body style="font-family: 'Segoe UI', Arial, sans-serif; background-color: #ffffff; color: #1f2937; line-height: 1.6; padding: 20px; margin: 0;">
  <div style="max-width: 700px; margin: 0 auto; background: #ffffff;">
    <div style="background: linear-gradient(135deg, #f59e0b, #d97706); padding: 24px; text-align: center; border-radius: 8px 8px 0 0;">
      <h1 style="color: #1f2937 !important; margin: 0; font-size: 24px;">🎭 ${guide.characterName}</h1>
      <p style="color: #374151 !important; margin: 8px 0 0 0;">${guide.productionTitle}</p>
    </div>
    <div style="padding: 24px; background: #ffffff; color: #1f2937 !important;">
      <style>
        h1, h2, h3, h4, h5, h6 { color: #b45309 !important; }
        p, li, span, div, td { color: #1f2937 !important; }
        strong, b { color: #92400e !important; }
    ...
    <div style="background: #f3f4f6; padding: 16px; text-align: center; border-radius: 0 0 8px 8px;">
      <p style="color: #6b7280 !important; margin: 0; font-size: 14px;">
        Generated by <strong style="color: #b45309 !important;">Prep101</strong> - Gold Standard Audition Preparation
      </p>
    </div>
  </div>
</body>

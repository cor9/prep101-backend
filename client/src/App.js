...
// ----------------- CORS -----------------
// CORRECTED: Added the live production URLs to the allowedOrigins list.
app.use(cors({
  origin: function (origin, callback) {
    ...
    const allowedOrigins = [
      ...
      'http://127.0.0.1:3001',
      'http://127.0.0.1:3002',
      // The key addition to fix the CORS error
      'https://prep101.site',
...
// CORRECTED: Changed endpoint to match the client.
// CORRECTED: Changed multer field name to 'file' to match the client.
app.post('/api/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    return res.json({
      ...
      // You may need to add more data here depending on your client's needs
    });
  } catch (e) {
    console.error('PDF parse/OCR error:', e);
    return res.status(500).json({ success: false, message: 'Failed to process PDF', error: e.message });
  }
});
...
app.post('/api/upload/text', (req, res) => {
  ...
  return res.json({ success: true, uploadId, textLength: text.length, filename: 'manual.txt' });
});
...
app.post('/api/guides/generate', async (req, res) => {
  ...
  if (!text) return res.status(400).json({ success: false, message: 'Upload a PDF or provide sceneText' });
  ...
  `;
  return res.json({ success: true, guideHtml });
});
